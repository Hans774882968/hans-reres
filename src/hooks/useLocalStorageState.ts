...
function useBrowserLocalStorageState<T> (
  ...
  localStorageSource: Window = window
): LocalStorageState<T | undefined> {
  const initialDefaultValue = useRef(defaultValue).current;

  const getLocalStorageObject = () => {
    return localStorageSource === window ? localStorage : localStorageSource.localStorage;
  };
  const localStorageMultiSource = getLocalStorageObject();
  ...
  if (
    ...
    localStorageMultiSource.getItem(key) === null
  ) {
    ...
    try {
      localStorageMultiSource.setItem(key, stringify(initialDefaultValue));
    } catch {}
  }
  ...
  const setState = useCallback(
    (newValue: SetStateAction<T | undefined>): void => {
      ...
      //   "SecurityError: The operation is insecure."
      try {
        localStorageMultiSource.setItem(key, stringify(value));
  ...
  useEffect(() => {
    ...
    const onStorage = (e: StorageEvent): void => {
      if (e.storageArea === localStorageMultiSource && e.key === key) {
        triggerCallbacks(key);
      }
    };
  ...
  return useMemo(
    () => [
      ...
      setState,
      {
        ...
        removeItem (): void {
          inMemoryData.delete(key);
          localStorageMultiSource.removeItem(key);
