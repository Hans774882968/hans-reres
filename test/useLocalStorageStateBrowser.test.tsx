...
describe('useLocalStorageState()', () => {
  ...
  describe('localStorageMultiSource', () => {
    test('defaultValue isn\'t provided', () => {
      const wnd: Window = { ...window };
      wnd.localStorage.setItem('todos', JSON.stringify([
        { name: 'first' },
        { name: 'second' }
      ]));
      const { result } = renderHook(() =>
        useLocalStorageState('todos', { localStorageSource: wnd })
      );
      const [todos] = result.current;
      ...
      expect(wnd.localStorage.getItem('todos')).toBe(todosString);
      expect(localStorage.getItem('todos')).toBe(todosString);
    });

    test('localStorage should not be overwritten if there is a value', () => {
      const wnd: Window = { ...window };
      wnd.localStorage.setItem('todos', JSON.stringify([
        { name: 'first' },
        { name: 'second' }
    ...
    test('wnd.localStorage !== localStorage, defaultValue is not provided', () => {
      ...
      expect(localStorage.getItem('color')).toBe(null);

      const { result } = renderHook(() =>
        useLocalStorageState('color', { localStorageSource: wnd })
      );
      act(() => {
        const setColor = result.current[1];
        setColor('pink');
      });

      expect(result.current[0]).toBe('pink');
      expect(wnd.localStorage.getItem('color')).toBe('"pink"');
      expect(localStorage.getItem('color')).toBe(null);
    });
    ...
    test('wnd.localStorage !== localStorage, defaultValue is provided', () => {
      ...
      expect(wnd.localStorage.getItem('colors')).toBe(JSON.stringify(['skyblue', 'deepskyblue']));
      expect(localStorage.getItem('colors')).toBe(null);

      act(() => {
        const setColors = result.current[1];
        setColors(['pink', 'red']);
      });
      expect(result.current[0]).toStrictEqual(['pink', 'red']);
      expect(wnd.localStorage.getItem('colors')).toBe(JSON.stringify(['pink', 'red']));
      expect(localStorage.getItem('colors')).toBe(null);
    });
  });
});
